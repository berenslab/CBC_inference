 ...
 */
FRAME *frame(char *afname), *_find(char **afname);
char *_fmatch(register char *fname, register char *fpath), *_path(register struct _frame *fp, register char *buf), *_cfname(char *fname);
double cwidth(double width, char *fname), xdist(double dx, char *fname), ydist(double dy, char *fname);
...
#define charw		(_dotp->_cw*0.65/_dotp->_sxcat)
