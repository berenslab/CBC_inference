...
void dashf(register float *ddefp)
 ...
 * of the prototype to the end of each of the 8 dash/space segments.
 */
   {
	register FRAME *dotp;
	register float *protop;	/* ptr to prototype */
	float tmp;
#	define NDASHLIN	8	/* predefined ddef[]s */
	static float preddef[] =
	  {	0.003,0.005,0.003,0.005,
		0.01,0.01,0.01,0.01,
		0.03,0.01,0.03,0.01,
		0.03,0.01,0.01,0.01,
	  	0.003,0.01,0.003,0.005,
		0.01,0.02,0.01,0.01,
		0.03,0.02,0.03,0.01,
		0.03,0.02,0.01,0.01,
	  };
	...
	if((long int)ddefp <= NDASHLIN)	/* use predefined dashes */
		ddefp = &preddef[(((long int)ddefp) - 1)*4];
	if (((long int)ddefp)&1) {		/* garbage ptr */
