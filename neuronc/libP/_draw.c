	  ...
	  {	_domove();
		if(dotp->_fflags&_DODASH)	/* do dash line */
		  {	register float *protop;	/* ptr to current proto segment */
			...
			extern double fmod(double da, double db), sqrt(double);
			...
			while (dr > (gap = *protop - dphase)) /* do complete segments */
			  {	dotp->_xcat += gap*dx;	/* go to end of this segment */
				dotp->_ycat += gap*dy;
				putc(*cmdp, rstdplt);
				putwsx(((short) dotp->_xcat), rstdplt);
				putwsx(((short) dotp->_ycat), rstdplt);
		...
		putc(*cmdp, rstdplt);
                _x = (long) xcatdraw;
                _y = (long) ycatdraw;
		putwsx((short)_x, rstdplt);
		putwsx((short)_y, rstdplt);
	  }
nodraw:	dotp->_x = xnew;
	dotp->_y = ynew;
	dotp->_xcat = xcatnew;
