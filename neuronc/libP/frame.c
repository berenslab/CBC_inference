 ...
 */
  {
	register FRAME *dotp;
	char *fname,*calloc(size_t, size_t);
	...
	while (*fname)		/* have to make a frame */
	  {	FRAME *olddotp;
		...
		olddotp = dotp;
		if ((dotp = (FRAME *)calloc(1, sizeof (FRAME))) == 0) {
			_err("frame: out of space\n");
		  }
		  {	int *ddp, *dp;

			for (dp = (int *)dotp, ddp = (int *)olddotp;
				dp < (int *)(dotp + 1); )
				*dp++ = *ddp++;
		  }
