...
void cpfir(void); /* make a table of firsts of nonterminals */
...
void others(void){ /* put out other arrays, copy the parsers */
	...
	if( finput == NULL ) error2a( "cannot find parser %s", PARSER );

	warray( "yyr1", levprd, nprod );

	aryfil( temp1, nprod, 0 );

/*	Modified code to work with new "/usr/lib/yaccpar".
 	Shift left and add one to reduction; 
	low bit of reduction is whether there
	is any action to be done on reduction (see yaccpar).
	So, set bit here so we always look for action even if there
	is none.  This is done because I don't know what code
	to add to y2.c to look for this bit when generating reduction.

	The old way:

	PLOOP(1,i)temp1[i] = prdptr[i+1]-prdptr[i]-2;	 */

	PLOOP(1,i)temp1[i] = ((prdptr[i+1]-prdptr[i]-2) << 1) + 1;
	...
	while( (c=getc(finput) ) != EOF ){
		if( c == '$' ){
			if( (c=getc(finput)) != 'A' ) putc( '$', ftable );
				...
				if( faction == NULL ) error1( "cannot reopen action tempfile" );
...
char *writem(int *pp) { /* creates output string for item pointed to by pp */
	...
	for(;;){
		...
		*q = '\0';
		if((i = *p) <= 0) break;
		q = chcopy( q, symnam(i) );
		if( q> &sarr[ISIZE-30] ) error1( "item too big" );
		}
...
void summary(void){ /* output the summary on the tty */
	...
	if( foutput!=NULL ){
		...
		fprintf( foutput, "memory: states,etc. %d/%d, parser %d/%d\n", (int)(zzmemsz-mem0), (int)MEMSIZE,
			    (int)(memp-amem), (int)ACTSIZE );
...
/* VARARGS1 */
void error3b(char *s, int a1, int a2) { /* write out error comment */
...
void error2a(char *s, char *a1) { /* write out error comment */
...
void error2b(char *s, int a1) { /* write out error comment */
...
void error1(char *s) { /* write out error comment */
...
void cpres(void){ 
	...
	NTLOOP(i){
		...
		if(pres[i] == pmem){
			error2a("nonterminal %s not defined!", nontrst[i].name);
			}
		}
	pres[i] = pmem;
	fatfl = 1;
	...
	if( pmem != &pyield[nprod] ) error2b( "internal Yacc error: pyield %d", pmem-&pyield[nprod] );
	}
...
int state(int c){ /* sorts last state,and sees if it equals earlier ones. returns state number */
	...
	/* state is new */
	if( nolook ) error1( "yacc state/nolook error" );
	pstate[nstate+2] = p2;
	if(nstate+1 >= NSTATES) error1("too many states" );
	if( c >= NTBASE ){
...
void putitem(int *ptr, struct looksets *lptr ) {
	...
	if( (int *)j > zzmemsz ){
		zzmemsz = (int *)j;
		if( zzmemsz >=  &mem0[MEMSIZE] ) error1( "out of state space" );
		}
	}
...
# define OK 1
	...
	NTLOOP(i){
		...
		if( pempty[ i ] != OK ) {
			fatfl = 0;
			error2a( "nonterminal %s never derives any token string", nontrst[i].name );
			}
		}
	...
	while( work ){
		...
		WSLOOP(wsets,u){
			...
			for( s=pres[c]; s<t; ++s ){
				...
				if( cwp-wsets+1 >= WSETSIZE ) error1( "working set overflow" );
				cwp->pitem = *s;
				cwp->flag = 1;
				if( !nolook ){
					work = 1;
...
struct looksets *flset(struct looksets *p ) {
	...
	/* add a new one */
	q = &lkst[nlset++];
	if( nlset >= LSETSIZE )error1("too many lookahead sets" );
	SETLOOP(j){
		q->lset[j] = p->lset[j];
		}
	return( q );
