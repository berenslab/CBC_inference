...
/* Rationale: implfh sets the mode of integration. 
    implfh = 0.5 -> Crank-Nicholson (second order)
...

{
  ...
  vm = pnt->v - pnt->vext - capnt->cavoff;
  implicitca = implicit;
  implfca = implfk;
  //implicitca = 1;
  //implfca = 1;
...
//ncfprintf (stderr,"111 t %g caflux %g caise0 %20.16g caise1 %20.16g caisn0 %20.16g casfn %20.16g implf0 %20.16g\n",
//		time,caflux,caise[0],caise[1],caisn[0],casfn,implf0);
     caise[0]= (cais[0]+caflux+(caise[1]+caisn[0])*casfn) / implf0;
     if (caise[0] < camind)  caise[0] = camind;

//ncfprintf (stderr,"222 t %g caflux %g caise0 %20.16g caise1 %20.16g caisn0 %20.16g casfn %20.16g implf0 %20.16g\n",
//		time,caflux,caise[0],caise[1],caisn[0],casfn,implf0);
...
/*   ncfprintf (stderr,"vrev %10.3g conduct %10.3g\n",
      ...
      case SYN2:
		...
		chpnt->conduct = conduct;
                nodcr += conduct * (chpnt->vrev - compcavoff(pnt));
      ...
      case NA:  hhpnt = (hhchan *)conpnt;
		...
		hhpnt->conduct = conduct;
                nodcr += conduct * (hhpnt->vrev - compcavoff(pnt));
                nodcur -= conduct * (pnt->v - vext(conpnt));  		/* current due to Vm */
                tcondn += conduct;    			/* total cond */
		break;
      ...
      case ClCa:   
		...
		kcapnts->conduct = conduct;
                nodcr += conduct * (kcapnts->vrev - compcavoff(pnt));
                nodcur -= conduct * (pnt->v - vext(conpnt));  			/* current due to Vm */
                tcondn += conduct;    			/* total cond */
		break;
      case KCa:   
		...
		kcapnt->conduct = conduct;
                nodcr += conduct * (kcapnt->vrev - compcavoff(pnt));
                nodcur -= conduct * (pnt->v - vext(conpnt));  			/* current due to Vm */
                tcondn += conduct;    			/* total cond */
		break;
...
/*  ncfprintf (stderr,"nodcr %10.3g nodcur %10.3g pntcond %g tcond %g\n",
 ...
 //pnt->implf = 1. / (1. + pnt->tcondt * pnt->k * implfk); /* implicit factor   */
   ...
   /* Crank-Nicolson,  or backward Euler fully implicit */
    ...
    pnt->nodcur = nodcur;			/* comp current recalc in dcomp() */
