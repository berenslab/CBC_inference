...
#define MAXERRCNT 100
  ...
  if (chpnt->nchan > 0) {			/* Noise on */
        ...
        int k,r,delch,nchano,totnchan,multp;
	double maxp, divp, totp;
		...
		/*  Then divide each rate by the maximum rate * 1.25 */ 
		/*  to make sure that the maximum rate (probability) is  */
		/*  is less than .5 so it doesn't get truncated in */
   ...
   multp = int(maxp*2.0 + 1.0);			/* max acceptable p = .5 */
   divp = 1.0 / multp;
   /* fprintf (stderr,"multp %d\n",multp); /* */
   ...
   for (r=0; r<multp; r++) {			/* repeat if p reduced */
    ...
    for (i=0; i<nstate; i++,stpnt++,conc++,nconc++){/* find change in conc */
      ...
      for (j=0; j<numtr; j++,trans++,tcrp++){     
        delch = int(binomdev(*tcrp*divp,conc->nchan)); 
/*  fprintf (stderr,"multp %d tcrp %g nchan %d del %d state %d tr %d\n",
