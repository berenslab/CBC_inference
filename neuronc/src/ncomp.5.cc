...
//fprintf (stderr,"\n");
    ...
    for (i=nstate-1; i>=0; i--,stpnt--,conc--,nconc--) { /* find change in conc */
      numtr = stpnt->numtrans;
      trans = stpnt->trans+numtr-1;               /* get parameters */
      tcrp = tcr[i]+numtr-1;                      /* pointer to precalc values */
      for (j=numtr-1; j>=0; j--,trans--,tcrp--){     
        if (chpnt->cstate) rsetstate (chpnt->cstate);
        delch = int(binomdev(*tcrp,conc->nchan)); 
//        delch = int(binomdev(*tcrp, int(chpnt->nchan*conc->cest))); 
//        newconc[*trans].dchan += delch;
