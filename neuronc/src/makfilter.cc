...
static double *xcoeffs, *ycoeffs;
double makfilter(uint options, int order, double a1, double a2, double *xc, double *yc);
...
static void expandpoly(double *xc, double *yc), expand(complex[], int, complex[]), multin(complex, int, complex[]);
static double printresults(void), printcmdline(char*[]), printfilter(), printgain(const char*, complex);
...
{
    // alpha = f(corner)/f(sample)\n\n", MAXORDER);
    raw_alpha1 = 2000.0/50000.0;
    raw_alpha2 = 0;
    order = 4;
    //options = opt_be | opt_lp;				// Bessel, low-pass 
    options = opt_be | opt_lp | opt_z;		// Bessel, low-pass, z-transform (linear phase)
     makfilter(options, order, raw_alpha1, raw_alpha2, xcoeffs, ycoeffs);
}

*/

double makfilter(uint opt, int ord, double a1, double a2, double *xc, double *yc)
{
    // readcmdline(argv);
    // checkoptions();
    options = opt;
...
static void expandpoly(double *xcoeffs, double *ycoeffs) 
...
static double printresults(void)
{ 
   complex gain;
