...
class OptimizerCOMSOLparams(Optimizer): 
  ...
  def clear(input=True, output=True):
    if input:
      COMSOL_inputs = [COMSOL_input for COMSOL_input in os.listdir(self.folder_i)]
      for COMSOL_input in COMSOL_inputs: os.remove(self.folder_i + COMSOL_input)
   
    if output:
      COMSOL_outputs = [COMSOL_output for COMSOL_output in os.listdir(self.folder_o)]
      for COMSOL_output in COMSOL_outputs: os.remove(self.folder_o + COMSOL_output)
  ...
  def create_RC_inputs(self):
    ...
    # To COMSOL.
    np.savetxt(f"{self.folder_i}/Rs.txt",  np.array(Rs),   delimiter=', ', newline='\n')
    np.savetxt(f"{self.folder_i}/Cs.txt",  np.array(Cs),   delimiter=', ', newline='\n')
  ...
  def create_inputs(self, opt_params=None, sim_params=None, verbose=True):
    ...
    if verbose: print("sig={:.3g} & eps={:.3g}".format(
        sim_params["sigma_retina"], sim_params["epsilon_retina"]), end=', ')
        
    np.savetxt(f"{self.folder_i}/sig.txt",
               np.array([[0, sim_params["sigma_retina"]], [1, sim_params["sigma_retina"]]]),
               delimiter=', ', newline='\n')
    
    np.savetxt(f"{self.folder_i}/er.txt",
               np.array([[0, sim_params["epsilon_retina"]], [1, sim_params["epsilon_retina"]]]),
               delimiter=', ', newline='\n')
