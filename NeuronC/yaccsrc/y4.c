...
void callopt(void){
	...
	if( (finput=fopen(TEMPNAME,"r")) == NULL ) error1( "optimizer cannot open tempfile" );
...
/*			fprintf( ftable, "%4d  ", p-a );	*/
			fprintf( ftable, "%6d  ", (int)(p-a));
/*			for( i=0; i<10; ++i ) fprintf( ftable, "%4d  ", p[i]);*/
			for( i=0; i<10; ++i ) fprintf( ftable, "%6d  ", p[i] );
			fprintf( ftable, "\n" );
			}
		}
...
void gin(int i){
	...
	for( p=a; p < &a[ACTSIZE]; ++p ){
		...
		for( r=q1; r<q2; r+=2 ){
			...
			if( *s ) goto nextgp;
			if( s > maxa ){
				if( (maxa=s) > &a[ACTSIZE] ) error1( "a array overflow" );
				}
			}
		...
		*p = *q2;
		if( p > maxa ){
			if( (maxa=p) > &a[ACTSIZE] ) error1( "a array overflow" );
			}
		for( r=q1; r<q2; r+=2 ){
			s = p + *r + 1;
			*s = r[1];
...
void stin(int i){
	...
	for( n= -maxoff; n<ACTSIZE; ++n ){
		...
		for( r = q1; r < q2; r += 2 ){
			if( (s = *r + n + a ) >= &a[ACTSIZE] ) error1( "out of space in optimizer a array" );
			if( s > maxa ) maxa = s;
			if( *s != 0 && *s != r[1] ) error3b( "clobber of a array, pos'n %d, by %d", s-a, r[1] );
			*s = r[1];
			}
		pa[i] = n;
	...
	error2b( "Error; failure to place state %d\n", i );
...
void osummary(void){
	...
	fprintf( foutput, "Optimizer space used: input %d/%d, output %d/%d\n",
		(int)(pmem-mem+1), (int)MEMSIZE, (int)(maxa-a+1), (int)(ACTSIZE));
...
int gtnm(void){
	...
	*pmem++ = s*val;
	if( pmem > &mem[MEMSIZE] ) error1( "out of space" );
	return( c );
