...
void error3b(char *s, int a1, int a2);
void error2a(char *s, char *a1);
void error2b(char *s, int a1);
void error1(char *s);
...
{	int i,j,lev,t, ty;
	...
	while( argc >= 2  && argv[1][0] == '-' ) {
		while( *++(argv[1]) ){
			...
			case 'V':
				...
				if( foutput == NULL ) error1( "cannot open y.output" );
				continue;
			case 'D':
			case 'd':
			...
			case 'r':
			case 'R':
				error1( "Ratfor Yacc is dead: sorry...\n" );

			default:
				error2b( "illegal option: %c", *argv[1]);
				}
			}
		argv++;
		argc--;
	...
	if( ftable == NULL ) error1( "cannot open table file" );

	ftemp = fopen( TEMPNAME, "w" );
	faction = fopen( ACTNAME, "w" );
	if( ftemp==NULL || faction==NULL ) error1( "cannot open temp file" );
	...
	for( t=gettok();  t!=MARK && t!= ENDFILE; ){
		...
		case TYPEDEF:
			if( (t=gettok()) != TYPENAME ) error1( "bad syntax in %%type" );
			ty = numbval;
			for(;;){
				t = gettok();
				switch( t ){
				...
				case IDENTIFIER:
					if( (t=chfind( 1, tokname ) ) < NTBASE ) {
						j = TYPE( toklev[t] );
						if( j!= 0 && j != ty ){
							error2a( "type redeclaration of token %s",
								tokset[t].name );
							}
						...
						if( j != 0 && j != ty ){
							error2a( "type redeclaration of nonterminal %s",
		...
		case TERM:
			...
			for(;;) {
				...
				case IDENTIFIER:
					j = chfind(0,tokname);
					if( lev ){
						if( ASSOC(toklev[j]) ) error2a( "redeclaration of precedence of %s", tokname );
						SETASC(toklev[j],lev);
						SETPLEV(toklev[j],i);
						}
					if( ty ){
						if( TYPE(toklev[j]) ) error2a( "redeclaration of type of %s", tokname );
						SETTYPE(toklev[j],ty);
						}
					if( (t=gettok()) == NUMBER ){
						tokset[j].value = numbval;
						if( j < ndefout && j>2 ){
							error2a( "please define type number of %s earlier",
								tokset[j].name );
							}
						t=gettok();
						}
...
/*		fprintf( ftable,  "extern int yychar;\nextern short yyerrflag;\n" ); */
	...
	*mem++ = 0;
	...
	if( t != C_IDENTIFIER ) error1( "bad syntax on first rule" );
	...
	while( t!=MARK && t!=ENDFILE ){
		...
		if( t == '|' ){
			...
			*mem = chfind(1,tokname);
			if( *mem < NTBASE ) error1( "token illegal on LHS of grammar rule" );
			++mem;
			}
		else error1( "illegal rule: missing semicolon or | ?" );
	...
	more_rule:
		...
		if( t == PREC ){
			if( gettok()!=IDENTIFIER) error1( "illegal %%prec syntax" );
			j = chfind(2,tokname);
			if( j>=NTBASE)error2a("nonterminal %s illegal after %%prec", nontrst[j-NTBASE].name);
		...
		if( t == '=' ){
			...
			if( (t=gettok()) == IDENTIFIER ){
				...
				*p++ = -nprod;
				...
				if( ++nprod >= NPROD ) error2b( "more than %d rules", NPROD );
				prdptr[nprod] = p;
		...
		*mem++ = -nprod;
		...
		if( ntypes && !(levprd[nprod]&ACTFLAG) && nontrst[*prdptr[nprod]-NTBASE].tvalue ){
			...
			if( tempty < 0 ) error1( "must return a value, since LHS has a type" );
			...
			if( tempty != nontrst[*prdptr[nprod]-NTBASE].tvalue ){
				error1( "default action causes potential type clash" );
				}
			}
...
/*	define s to be a terminal if t=0
	...
	if (t) {
		if( ++nnonter >= NNONTERM ) error2b("too many nonterminals, limit %d",NNONTERM);
		nontrst[nnonter].name = cstash(s);
		return( NTBASE + nnonter );
		}
	/* must be a token */
	if( ++ntokens >= NTERMS ) error2b("too many terminals, limit %d",NTERMS );
	...
	if( s[0]==' ' && s[2]=='\0' ) /* single character literal */
		...
		if( s[3] == '\0' ){ /* single character escape sequence */
			...
			case '"': val = '"'; break;
			case '\\': val = '\\'; break;
			default: error1( "invalid escape" );
				}
			}
		else if( s[2] <= '7' && s[2]>='0' ){ /* \nnn sequence */
			if( s[3]<'0' || s[3] > '7' || s[4]<'0' ||
				s[4]>'7' || s[5] != '\0' ) error1("illegal \\nnn construction" );
			val = 64*s[2] + 8*s[3] + s[4] - 73*'0';
			if( val == 0 ) error1( "'\\000' is illegal" );
			}
		}
	else {
		val = extval++;
...
cstash( s ) register char *s; {
	...
	temp = cnamp;
	do {
		if( cnamp >= &cnames[cnamsz] ) error1("too many characters in id's and literals" );
		else *cnamp++ = *s;
		}  while ( *s++ );
	return( temp );
	}
...
begin:
	...
	case '<':  /* get, and look up, a type name (union member name) */
		...
		if( c != '>' ) error1( "unterminated < ... > clause" );
		tokname[i] = '\0';
		for( i=1; i<=ntypes; ++i ){
	...
	if( reserve ){ /* find a reserved word */
		...
		error2a("invalid escape, or illegal reserved word: %s", tokname );
		}
...
int fdtype( int t ){ /* determine the type of a symbol */
	...
	if( v <= 0 ) error2a( "must specify type for %s", (t>=NTBASE)?nontrst[t-NTBASE].name:
			tokset[t].name );
	return( v );
	}
...
int chfind(int t, register char *s ) {
	...
	/* cannot find name */
	if( t>1 )
		error2a( "%s should have been defined earlier", s );
	return( defin( t, s ) );
	}
...
void cpyunion(void){
	...
	level = 0;
	for(;;){
		if( (c=getc(finput)) < 0 ) error1( "EOF encountered while processing %%union" );
		putc( c, ftable );
...
int skipcom(void){ /* skip over comments */
	...
	if( getc(finput) != '*' ) error1( "illegal comment" );
	c = getc(finput);
	while( c != EOF ){
		while( c == '*' ){
...
case '$':
		...
		if( c == '<' ){ /* type description */
			ungetc( c, finput );
			if( gettok() != TYPENAME ) error1( "bad syntax on $<ident> clause" );
			tok = numbval;
			c = getc(finput);
			}
		if( c == '$' ){
		...
		if( isdigit(c) ){
			...
			j = j*s - offset;
			if( j > 0 ){
				error2b( "Illegal use of $%d", j+offset );
				}

			fprintf( faction, "yypvt[-%d]", -j );
			if( ntypes ){ /* put out the proper tag */
				if( j+offset <= 0 && tok < 0 ) error2b( "must specify type of $%d", j+offset );
...
case '"':	/* character string */
	...
	string:
		...
		while( c=getc(finput) ){
			...
			else if( c=='\n' ) error1( "newline in string or char. const." );
			putc( c , faction );
			}
		error1( "EOF in string or character constant" );

case EOF:
		error1("action does not terminate" );
