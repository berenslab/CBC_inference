...
** Modified July 1992 to avoid multiple minima if waveform has 
...
typedef struct spk_type {
	int ipeak,ivalley;
	double peak,valley;
	double tpeak,tvalley;
	double isi,ptp;
} SPK;

#define SPIKCRIT (-0.01)

static double crit=SPIKCRIT;
	...
	char	**argv;
{
	...
	double	tsumsq=0.0,psumsq=0.0,ssumsq=0.0,nsumsq=0.0;
	double	ftemp,foffset,interp,scale;
	int		itemp;
	int		nspk,npts1,npts2,nspk1=0,nspk2=0;
	double	sqrt();
	int		i,j,k;
	double	t,st,interp1,interp2;
	FILE	*out1,*out2;
	double	dt;
	...
	for(i=3;i<npts1-2;i++) {
		if (v1[i-2]<v1[i] && v1[i-1]<=v1[i] &&
			v1[i]>v1[i+1] && v1[i]>v1[i+2] && v1[i]>crit) {
			spk1[nspk1].ipeak=i;
	...
	for(i=3;i<npts2-2;i++) {
		if (v2[i-2]<v2[i] && v2[i-1]<=v2[i] &&
			v2[i]>v2[i+1] && v2[i]>v2[i+2] && v2[i]>crit) {
			spk2[nspk2].ipeak=i;
 ...
 /* printf ("i %d ptp %g tim %g\n",i,spk2[i].ptp,spk2[i].tpeak);
*/
		/* Find the fractional difference */
