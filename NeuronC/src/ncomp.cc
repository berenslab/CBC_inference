...
int *domainvcs = NULL;			// array of domains for vc 
...
{
   static int i,j;
   static int t,nstate,numtr,errcnt;
   ...
   static double *trmul;
   static char *rateo;
   static double trm;			/* multiplier for rate func */
   static char *trans;
   static double cerr,cmaxerr,cest;
...
*/
  ...
  if (chpnt->nchan > 0) {			/* Noise on */
        ...
        static int k,r,dchan,delch,nchano,totnchan,multp;
	static double maxp, divp, totp;
   ...
   for (r=0; r<multp; r++) {			/* repeat if p reduced */
	...
	static double stay_trans;
       	static double cum_trans;
       	static double cum_tr[NUMTRANS];
       	static double p;
	static int k;
...
{  
     double flux; // flux in terms of [Ca]/time total volume
     double cais_global = cais*(1/(1 + capnt->c1cicr)); 
	//[cais] uses cytoplasmic volume, conversion to total cellular volume ( constants in terms of total volume)

  flux = capnt->c1cicr * capnt->vm2 * pow(cais_global,capnt->ncicr) / 
			           (pow(capnt->k2cicr,capnt->ncicr) + pow(cais_global,capnt->ncicr)); 

  return flux*(1+ capnt->c1cicr);  //convert [Ca]/time to Cai cytoplasmic volume
}
...
{ 
     double flux;  		// flux in terms of [Ca]/time total cellular volume

     double cais_global; 	//[cais] uses cytoplasmic volume, 
				// conversion to total cellular volume ( constants in terms of total volume)

     double cas_global; 	// [ca] is local store volume, 
				//constants are written in terms of total cellular volume (adjustment provided)

  cais_global = cais*(1/(1 +  capnt->c1cicr)); 
  cas_global  = cas *(1/(1+(1/capnt->c1cicr)));

  flux = capnt->c1cicr * (capnt->vm3 * pow(cas_global, capnt->mcicr) / 
				    (pow(capnt->krcicr,capnt->mcicr) + pow(cas_global,capnt->mcicr)) * 
                                       pow(cais_global,capnt->pcicr) / 
				      (pow(cais_global,capnt->pcicr) + pow(capnt->kacicr,capnt->pcicr)) +
				 	  capnt->kfcicr *cas_global);
  
  return flux*(1+capnt->c1cicr); //convert total [Ca]/time to Cai cytoplasmic volume
} 
 ...
 
{
  ...
  ip3Minf = (capnt->ip3i/(capnt->ip3i + capnt->d1ip3)) * 
			  (cais/(cais + capnt->d4ip3));
 // ncfprintf(stderr,"m_inf = %e\n",ip3Minf);
  ...
  if (capnt->mtypeip3 == 0){
      ...
      m = capnt->mip3 + (1- implfk)*(mest - capnt->mip3) + implfk*dmest;
  }
  capnt->mip3 = m;
  return m;
}
...
{
      double h, hest, dhest, ip3Hinf, Qfactor,htest,dhtest,hval;

  Qfactor = capnt->d2ip3*(capnt->ip3i+capnt->d1ip3) / (capnt->ip3i + capnt->d3ip3);
  ip3Hinf = Qfactor/(Qfactor + cais);
  hest = capnt->hip3 + timinc*(ip3Hinf - capnt->hip3) / (ip3TauH(capnt,cais,Qfactor));
  dhest = timinc*(ip3Hinf-hest) / (ip3TauH(capnt,cais,Qfactor));
  h = capnt->hip3 + (1-0.5)*(hest - capnt->hip3) + 0.5*dhest;
  capnt->hip3temp = h;
  return h;
}
...
{
     double flux; //flux in terms of local cyto volume (constants based on local volume)

     double firstpart = capnt->v4ip3*pow(ip3M(capnt,cais),3) * pow(ip3H(capnt,cais),3)*(cas2-cais); 
     double secondpart = capnt->v2ip3*(cas2-cais);

  flux = (capnt->c1cicr/(1+ capnt->c1cicr))*(firstpart + secondpart);
...
{
     double flux;   //flux is in terms of local cyto volume (constants in terms of local volume - no conversion)
...
/* ncfprintf (stderr,"ica %g pica %g eica %g ina %g\n",ica,pica,eica,ina); /* */
  ...
  if ((capnt->vm3 > 0) || (capnt->v4ip3 > 0) || (capnt->v2ip3 > 0)) { 
...
//    printf("ip3_u %e",ip3_u);
//    printf(" ip3_r %e\n",ip3_r);

//    double ip3_u_test = ip3fluxu(capnt,0.05e-6);
//    double ip3_r_test = ip3flux(capnt,10.5e-6,0.05e-6);
//    ncfprintf(stderr,"ip3 uptake for 10.5e-6 cas2, 0.05e-6 cais = %e\n",ip3_u_test);
//    ncfprintf(stderr,"ip3 release for 10.5e-6 cas2, 0.05e-6 cais = %e\n",ip3_r_test);

      /* make initial estimate for beginning of time step */
      /* dcais calib in M/L */

    dcais = timinc * ((cicr_r - cicr_u +  
			  ip3_r - ip3_u) - capnt->k1cicr*cais0 + capnt->vip3*capnt->bip3);

    dcas  = timinc *    (cicr_u - cicr_r)/capnt->c1cicr;  //convert to store volume
    dcas2 = timinc * (ip3_u - ip3_r)/capnt->c1cicr;  
  
    dcas_est = dcas;		/* save for implicit est */
    dcais_est  = dcais;
    dcas2_est  = dcas2;

 //   double hip3_inter = capnt->hip3temp;
      /* make new estimate for end of time step, iterate for implicit */

    cacrit = 1e-10;
/*    ncfprintf(stderr,"cas = %e\n",capnt->cas);
    printf("cais0 = %e\n", cais0);
    printf("cas = %e\n",capnt->cas);
    printf("cas2 = %e\n",capnt->cas2);  */

    capnt->hip3 = capnt->hip3temp;
    ...
    for (n=0; n<100 && dcais_err > cacrit; n++) {
        ...
        dcais = timinc * ((cicr_r - cicr_u + ip3_r - ip3_u) - 
	   capnt->k1cicr*tempcais  + capnt->vip3*capnt->bip3);

	dcas_est  = timinc * (cicr_u - cicr_r)/capnt->c1cicr;  //convert conc in terms of store vol
        dcas2_est = timinc * (ip3_u - ip3_r)/capnt->c1cicr;  //convert conc in terms of store volume

        dcais_err = ncabs(dcas_est - dcas);
	capnt->hip3 =capnt->hip3temp;
    }
...
int set_domain(comp *cpnt, int domain)

/* Find a domain defined by resistively coupled compartments
and set the domain number in each one. If the domain number 
for a compartment has already been set, stop and return the 
number of compartments. This procedure is recursively called.
*/

{
  ...
  n = 0;
  if (cpnt->domain > 0) return n; 
  else cpnt->domain = domain;
  n++;

  for (c=cpnt->clst; c!=NULL; c=c->next) {
      switch (c->conpnt->ctype) {
         case AXIALRES: case RESISTOR: case GJ: 
	 case PNX: case CAP: case BATT: break; 
         default: continue; break;
      }

      if (cpnt==c->conpnt->comp1) {
	 n += set_domain(c->conpnt->comp2,domain);
      } else {
	 n += set_domain(c->conpnt->comp1,domain);
      } 
  }
...
void setdomains(void)

/* Find compartments resistively coupled, label with domain number. */
/* Make an array to hold vc status for each domain */

{
    int domain, ncomps;
    extern int ndomains;
    extern int *domainvcs;
    comp *pnt;

  for (pnt=compnt, domain=1; pnt; pnt=pnt->next) {
     if ((ncomps=set_domain(pnt, domain)) > 0) {
          if (debug & NCOMP && debugz & 2)
              ncfprintf (stderr,"first comp %d domain %d ncomps %d\n",
	                       pnt->num,domain,ncomps);
          domain++;
     }
  }
  ndomains  = domain - 1;
  if (domainvcs != NULL) efree (domainvcs);
  domainvcs = (int *)emalloc((ndomains+2) * sizeof(int));
}
...
// implfk = .5;
 ...
 for (pnt=compnt; pnt; pnt=pnt->next) {
   ...
   pnt->oldvext = pnt->vext;	/* save old voltage, allows save/restore */
   pnt->extv_old = 0;	        /* save previous vc voltage */
 }

/*
 if (domainvcs == NULL) 
     ncfprintf (stderr,"domainvcs not allocated.\n");
 else
    for (i=0; i<ndomains; i++) {	// reset domain vcs 
        domainvcs[i] = 0;
    }
/* */
  ...
  {                                                     /* for all comps */
   ...
   for (lpnt=pnt->clst; lpnt; lpnt=lpnt->next) 
    {                                           /* check all connections */
     ...
     if (!ocpntf)
      switch (conpnt->ctype)
      {                         /* add connection conductances that change */
...
/*   ncfprintf (stderr,"vrev %10.3g conduct %10.3g\n",
      ...
      case AMPA:
      case KAINATE:
      case GABA:		/* Chans without Ca current */
      case GLY:
      case SYN2:
		chpnt = (chan *)conpnt;
  		conduct = dochan2(chpnt);
...
/* ncfprintf (stderr,"conduct %g ipump %g\n",conduct,capnt->ipump); /* */
   ...
   //
   if (pnt->pvext!=NULL) nodcr += (pnt->vext - pnt->oldvext) * pnt->cap / timinc;
   //
   ...
   nodcr  += (pnt->vrev + pnt->vext -compcavoff(pnt)) * pnt->rm;  /* membrane conductance */
...
/*  ncfprintf (stderr,"nodcr %10.3g nodcur %10.3g pntcond %g tcond %g\n",
 ...
 if (euler)	{     			      /* forward-Euler */
     ...
     if (pnt->miscfl & VEXT) {                /* comp is a battery */
       ...
       vc |= (pnt->extv != pnt->extv_old);    /* save voltage clamp change */
       // domainvcs[pnt->domain] |= (pnt->extv != pnt->extv_old);    /* save voltage clamp change */
       pnt->extv_old = pnt->extv;	      /* save previous vc voltage */
     }
     else {
       pnt->extvi = 0.0;
     }
 ...
 //pnt->implf = 1. / (1. + pnt->tcondt * pnt->k * implfk); /* implicit factor   */
   ...
   /* Crank-Nicolson,  or backward Euler fully implicit */
    ...
    pnt->vest = pnt->v + nodcur * pnt->k;       /* simple first-order est */
    if (pnt->miscfl & VEXT) {                   /* comp is a battery */
...
#endif
       if (noskip==0 && !vc) 
       // if (noskip==0 && !(domainvcs[pnt->domain])) 
...
void docomp(register comp *pnt)
{
  ...
  int i;
     ...
     /*  Can change implfk during iteration if necessary */
