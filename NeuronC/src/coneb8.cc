...
main(void)
{
 ...
 rk_b  = 2.0;		/* bleaching recovery, see cone_ode.cc */
 tau_b = 250000;	/* time const for bleaching recovery */
 tau_r = 34;		/* R* lifetime */
 cn    = 4e-9*tau_r/34;
 ...
 for (i=0,j=plotinc; i<2500; i++) {
  ...
  stim = 0;
  if (i == 500) stim += 10000.0;				/* */
/*  if (i > 400  && i <= 600) stim = 100.0 * lgain;	/* */
/*  if (i <= 400) stim = 1.0 * lgain;			/* */
/*  stim = .1 * lgain;					/* */

  rhod += stim * lgain * (1 - bl);				/* */
//  rhod += (stim * lgain);

  bl += cn*rhod/tau_r - rk_b/tau_b * bl/(bl+rk_b);
  ...
  if (++j >= plotinc) {
	j = 0;
	printf ("%g %10.9g %8.5g %8.5g %8.5g %8.5g %8.5g %8.5g %8.5g\n",
			i/plotinc,cond,bl,pde,gcyc,cycg,ca,cak,rhodk);
